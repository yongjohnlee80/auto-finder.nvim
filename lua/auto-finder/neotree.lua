...
--- ```lua
--- require("auto-finder.neotree.command").execute({ action = "close" })
--- ```
---@deprecated
...
M.ensure_config = function()
  if not M.config or new_user_config then
    M.config = require("auto-finder.neotree.setup").merge_config(new_user_config)
    new_user_config = nil
  end
  return M.config
end
...
M.set_log_level = function(level)
  require("auto-finder.neotree.log").set_level(level)
end
...
local function try_netrw_hijack(path)
  ...
  return require("auto-finder.neotree.setup.netrw").hijack()
end
...
M.setup = function(config)
  ...
  require("auto-finder.neotree.log").use_file(new_user_config.log_to_file, true)
...
M.show_logs = function()
  vim.cmd("tabnew " .. require("auto-finder.neotree.log").outfile)
end
