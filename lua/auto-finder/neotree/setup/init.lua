...
local defaults = require("auto-finder.neotree.defaults")
local mapping_helper = require("auto-finder.neotree.setup.mapping-helper")
...
local netrw = require("auto-finder.neotree.setup.netrw")
local hijack_cursor = require("auto-finder.neotree.sources.common.hijack_cursor")
...
local define_events = function()
  ...
  events.subscribe({
    ...
    handler = function()
      require("auto-finder.neotree.ui.renderer").update_floating_window_layouts()
    end,
  })
end
    ...
    if position == "current" then
      ...
      if neo_tree_winid ~= current_winid then
        ...
        vim.schedule(function()
          ...
          require("auto-finder.neotree.sources.filesystem")._navigate_internal(state, nil, nil, nil, false)
        end)
        return
      end
    end
...
M.merge_config = function(user_config)
  ...
  local migrations = require("auto-finder.neotree.setup.deprecations").migrate(user_config)
  ...
  for _, source in ipairs(user_config.sources or default_config.sources or {}) do
    ...
    if #parts == 1 then
      ...
      is_internal_ns, module = pcall(require, "auto-finder.neotree.sources." .. source)
    end
    if is_internal_ns then
      name = module.name or name
      all_sources[name] = "auto-finder.neotree.sources." .. name
    else
  ...
  require("auto-finder.neotree.command.parser").setup(all_source_names)
  ...
  elseif hijack_behavior ~= "disabled" then
    require("auto-finder.neotree.log").error(
  ...
  require("auto-finder.neotree.ui.renderer").resize_timer_interval = rt

  if M.config.enable_cursor_hijack then
    hijack_cursor.setup()
  end

  require("auto-finder.neotree.clipboard").setup(M.config.clipboard)
