...
local Backend = require("auto-finder.neotree.clipboard.sync.base")
local log = require("auto-finder.neotree.log").new("clipboard")
...
function UniversalBackend:_watch_file(filename)
  ...
  local start_success = event_handle:start(filename, {}, function(err)
    ...
    require("auto-finder.neotree.clipboard").update_states()
  end)
...
function UniversalBackend:load(state)
  ...
  if not is_success or not validate_clipboard_from_file(saved_clipboard) then
    ...
    if
      require("auto-finder.neotree.ui.inputs").confirm(
