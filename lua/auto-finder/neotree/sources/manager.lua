...
local compat = require("auto-finder.neotree.utils._compat")
...
local inputs = require("auto-finder.neotree.ui.inputs")
...
M.navigate = function(state_or_source_name, path, path_to_reveal, callback, async)
  require("auto-finder.neotree").ensure_config()
  ...
  if not mod then
    mod = require("auto-finder.neotree.sources." .. source_name)
  end
...
--- ```lua
--- require("auto-finder.neotree.command").execute({ source_name = source_name, action = "focus", reveal = true })` instead
--- ```
M.reveal_current_file = function(source_name, callback, force_cwd)
  log.warn(
    [[DEPRECATED: use `require("auto-finder.neotree.command").execute({ source_name = source_name, action = "focus", reveal = true })` instead]]
  )
...
--- To be removed in 4.0. Use:
--- ```lua
--- require("auto-finder.neotree.command").execute({ source_name = source_name, action = "focus", reveal = true, position = "current" }
--- ```
--- instead.
M.reveal_in_split = function(source_name, callback)
  log.warn(
    [[DEPRECATED: use `require("auto-finder.neotree.command").execute({ source_name = source_name, action = "focus", reveal = true, position = "current" })` instead]]
  )
...
local validate = require("auto-finder.neotree.health.typecheck").validate
---@param source_name string
...
M.setup = function(source_name, config, global_config, module)
  ...
  if module == nil then
    module = require("auto-finder.neotree.sources." .. source_name)
  end
