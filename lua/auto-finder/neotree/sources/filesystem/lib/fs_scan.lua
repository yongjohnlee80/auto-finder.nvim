...
local filter_external = require("auto-finder.neotree.sources.filesystem.lib.filter_external")
...
local ignored = require("auto-finder.neotree.sources.filesystem.lib.ignored")
...
local mark_gitignored = function(cwd, items)
  ...
  for worktree_root, data in pairs(worktree_data) do
    ...
    if not git_status then
      ...
      local ignored_paths = require("auto-finder.neotree.git.check-ignore").check(worktree_root, paths)
...
local function async_scan(context, path)
  ...
  local scan_mode = require("auto-finder.neotree").config.filesystem.scan_mode
