...
local fs_actions = require("auto-finder.neotree.sources.filesystem.lib.fs_actions")
...
local popups = require("auto-finder.neotree.ui.popups")
local log = require("auto-finder.neotree.log")
local help = require("auto-finder.neotree.sources.common.help")
local Preview = require("auto-finder.neotree.sources.common.preview")
local async = require("plenary.async")
local git = require("auto-finder.neotree.git")
local node_expander = require("auto-finder.neotree.sources.common.node_expander")
...
local function get_folder_node(state)
  ...
  while node do
    ...
    local insert_as_global = require("auto-finder.neotree").config.window.insert_as
    local use_parent
...
local get_sources = function()
  local config = require("auto-finder.neotree").config
...
M.order_by_created = function(state)
  ...
  require("auto-finder.neotree.sources.manager").refresh(state.name)
end
...
M.paste_from_clipboard = function(state, callback)
  ...
  paste_complete = function(source, destination)
    if callback then
      local insert_as = require("auto-finder.neotree").config.window.insert_as
