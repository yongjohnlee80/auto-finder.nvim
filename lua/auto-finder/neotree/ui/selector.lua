...
local calc_click_id_from_source = function(winid, source_index)
  local base_number = #require("auto-finder.neotree").config.source_selector.sources + 1
...
M.get_scrolled_off_node_text = function(state)
  if state == nil then
    state = require("auto-finder.neotree.sources.manager").get_state_for_window()
    if state == nil then
      return
    end
  end
...
M.get = function()
  local state = require("auto-finder.neotree.sources.manager").get_state_for_window()
  if state == nil then
    return
  else
    local config = require("auto-finder.neotree").config
...
M.set_source_selector = function(state)
  ...
  local sel_config = utils.resolve_config_option(require("auto-finder.neotree").config, "source_selector", {})
  if sel_config and sel_config.winbar then
    vim.wo[state.winid].winbar = "%{%v:lua.require 'auto-finder.neotree.ui.selector'.get()%}"
  end
  if sel_config and sel_config.statusline then
    vim.wo[state.winid].statusline = "%{%v:lua.require 'auto-finder.neotree.ui.selector'.get()%}"
  end
end
...
_G.___neotree_selector_click = function(id, _, _, _)
  ...
  local sources = require("auto-finder.neotree").config.source_selector.sources or {}
